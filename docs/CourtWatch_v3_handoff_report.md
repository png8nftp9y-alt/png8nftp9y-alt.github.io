...
## Cronologia recuperata dalla storia Git — 5–12 settembre 2026

### 2026-09-05 — Mark account and session phase complete (`e130b5e5`)

## Revisione 2026-09-05.137 — completamento fase account e sessione
- Il deploy consolidato `33950629301` è verde: migrazioni D1, import universale, gate di proprietà account/giocatori/analisi, validazione Worker, pubblicazione e parità API live sono tutti riusciti.
- La protezione senza sessione è stata verificata esternamente: Cloudflare Access intercetta `/app` prima che l'applicazione o i dati vengano serviti.
- L'account Federico è attivo e la completezza dei dati per `user_id` è certificata dal gate D1. La vecchia password dell'icona analisi non è più utilizzata dal client.
- Il polling client a 30 secondi è eliminato e tutte le risposte protette sono `no-store`. Non risultano chiamate a provider AI nel client o nel Worker.
- La fase tecnica login/sessione/account/migrazione è conclusa. È possibile proseguire con il lavoro grafico.

### 2026-09-05 — Record blocked CourtWatch account access (`0294803e`)

## Revisione 2026-09-05.138 — accesso account non ancora operativo
- Rettifica della revisione precedente: il blocco anonimo e la migrazione D1 sono verdi, ma il collaudo autenticato non è concluso e non si deve ancora passare alla grafica.
- Causa verificata: la pagina Access espone soltanto il provider Cloudflare, non il campo email One-Time PIN. La policy `Federico only` da sola non rende disponibile un metodo di autenticazione utilizzabile dall'account previsto.
- Preparata la configurazione idempotente del provider `onetimepin` e l'associazione esclusiva dello stesso provider alle applicazioni `/app` e `/app-api`.
- Il run `33966250515` è stato bloccato con HTTP 403 sulla lettura degli Identity Provider: il secret `CLOUDFLARE_ACCESS_API_TOKEN` non possiede il permesso account necessario per gestire `Access: Organizations, Identity Providers, and Groups`.
- Azione esterna necessaria: estendere o sostituire quel token con permesso Edit per Identity Providers, mantenendo il permesso Edit per Access Apps and Policies. Dopo l'aggiornamento del secret, il workflow deve essere rieseguito e il login OTP verificato realmente.

### 2026-09-05 — Record successful authenticated CourtWatch access (`eb0ae246`)

## Revisione 2026-09-05.139 — accesso account corretto e collaudato
- Il secret GitHub `CLOUDFLARE_ACCESS_API_TOKEN` è stato sostituito con un token dotato dei permessi necessari per gestire gli Identity Provider.
- Il secondo tentativo del workflow `33966250515` è verde. È stato creato/riutilizzato il provider Cloudflare Access `onetimepin` e associato in modo esclusivo alle applicazioni protette `/app` e `/app-api`.
- Verifica anonima aggiornata: la pagina non serve l'app e presenta il campo Email con il comando `Send login code`, senza proporre il precedente login tramite account Cloudflare.
- Verifica autenticata reale completata con il codice monouso inviato all'email autorizzata: redirect riuscito a `/app`, titolo `Court Watch V3`, agenda e calendario caricati.
- La proiezione autenticata mostra tutti i 23 giocatori previsti. Il gate D1 continua inoltre a imporre parità completa tra giocatori applicativi e collegamenti dell'utente, e tra analisi legacy e analisi per utente.
- Il problema “that account does not have any access” è risolto. La fase login/sessione/account/migrazione è ora effettivamente conclusa e si può passare alla grafica.

### 2026-09-05 — Record exact pre-login app restoration (`8dc8c953`)

## Revisione 2026-09-05.140 — ripristino integrale interfaccia pre-login
- Ripristinati `v3.html` e `v3.js` dalla revisione immediatamente precedente al lavoro sul login (`75383f7d78132990f7ec8852a645173385dc4614`).
- Conservate esclusivamente le integrazioni obbligatorie: redirect verso `/app`, endpoint dati e analisi protetti per utente e rimozione della seconda password dell'icona analisi.
- Ripristinato anche il comportamento client originario; aggiornato il cache-buster JavaScript per impedire l'esecuzione di copie incompatibili rimaste nella cache del browser.
...
### 2026-09-05 — Document protected app routing regression fix (`d9523474`)
...
### 2026-09-05 — Document effective client cache invalidation (`db640040`)
...
### 2026-09-05 — Document restored pre-login ITF agenda projection (`217b9df3`)
...
### 2026-09-05 — Document definitive Home Today navigation fix (`8c7289e2`)
...
### 2026-09-05 — Document extended functional QA and remaining analysis UI check (`6e754fd6`)
...
### 2026-09-05 — Document complete agenda venue rendering (`7982bdbb`)
...
### 2026-09-05 — Record green analysis CRUD deployment gate (`7883cc32`)
...
### 2026-09-05 — docs: record analysis editor verification status (`7d28c656`)

## Revisione 2026-09-05.148 — editor analisi interno e gate permanente
- Eliminati i dialoghi nativi `prompt()` e `confirm()` dal flusso analisi. L'icona apre ora un editor interno all'app con textarea e comandi `Salva`, `Elimina`, `Annulla` e `Chiudi`; non viene richiesta alcuna seconda password.
- Lettura, creazione, modifica ed eliminazione continuano a usare esclusivamente gli endpoint protetti per utente `/app-api`. La sessione Cloudflare Access resta l'unica autenticazione.
- Aggiunto `scripts/verify-analysis-ui.mjs` al comando CI `npm run check`: il deploy viene bloccato se ricompaiono prompt/confirm nativi, se manca l'editor interno, se spariscono PUT/DELETE o se l'origine API torna relativa al tag `base`.
- Deploy dell'interfaccia verde: run `33970137290`.
- Il run D1/Worker `33970228217`, che include il nuovo gate, risultava ancora in esecuzione all'unico controllo post-deploy. Nessun polling è stato eseguito.
- Il browser autenticato automatico non era disponibile nell'ambiente di collaudo al momento della verifica; pertanto il CRUD UI reale non viene dichiarato completato e non viene ancora attestata la piena equivalenza funzionale pre-login.

### 2026-09-05 — docs: record final authenticated app acceptance (`d37e15da`)

## Revisione 2026-09-05.149 — collaudo finale login e parità funzionale
- Collaudo conclusivo completato sulla versione pubblicata. L'utente ha verificato positivamente nell'app autenticata: `Home · Oggi`, navigazione interna, luogo agenda e comportamento delle tendine partite.
- Le tendine delle partite sono chiuse al primo ingresso nella pagina giocatore; dopo l'apertura restano aperte durante gli aggiornamenti automatici e il refresh della stessa pagina; lo stato viene cancellato quando si esce verso Home, un torneo o un altro giocatore.
- Il polling dati dell'app ogni 30 secondi resta attivo. La limitazione dei controlli ripetuti riguarda esclusivamente il monitoraggio dell'assistente sui run GitHub.
- L'analisi della singola partita è operativa: usa l'editor interno, non richiede una seconda password e comunica con `/app/api`, sotto la stessa sessione Cloudflare Access della pagina.
- Il luogo agenda usa `circolo · località`; quando la sorgente include il circolo come prefisso della località, viene rimosso soltanto il duplicato e vengono conservati città e Paese. Il testo non viene troncato.
- Verifica statica della versione pubblicata: nessun flusso password analisi; editor presente; `Home · Oggi` usa URL assoluto; stato tendine salvato e cancellato all'uscita; nessuna chiusura forzata residua; cache `v3.js?v=2026090516`, `v3.css?v=2026090504`.
- Verifica anonima esterna: `/app` risponde con redirect `302` alla pagina Cloudflare Access e non serve l'applicazione.
- Run certificati verdi: D1/Worker `33972476098`, Agenda Europe D1 `33972951456`, deploy `33972928678` e deploy della gestione tendine `33972765908`.
- Login/sessione, account, migrazione dati, parità funzionale pre-login, analisi match e collaudo finale sono conclusi. È possibile passare alla parte grafica.

### 2026-09-05 — docs: record account menu and full-name migration (`df7f10ad`)

## Revisione 2026-09-05.150 — menu account e nome completo
- Aggiunta in alto a destra l'icona circolare dell'account. Le iniziali sono ricavate dalla sessione protetta `/app/api/session`; cliccando l'icona si apre un menu a tendina con la voce `Logout`.
- Il logout usa `location.origin + '/cdn-cgi/access/logout'`, quindi termina la sessione Cloudflare Access sull'origine corretta anche in presenza del tag `base` per gli asset.
- Il nome completo indicato dall'utente è `Federico Quadri`; aggiunta la migrazione conservativa `0013_federico_quadri_display_name.sql`. Una volta applicata, l'avatar mostra `FQ`.
- Interfaccia pubblicata con i run verdi Pages `33974061665` e Deploy CourtWatch `33974062274`.
- La migrazione D1 era ancora in esecuzione nel run `33974062257` all'ultimo controllo; l'attivazione del nome completo non viene dichiarata conclusa finché quel run non termina verde.

### 2026-09-05 — docs: record FQ account and calendar hold filter (`0709e05e`)

## Revisione 2026-09-05.151 — account FQ e filtro calendario con pressione prolungata
- La migrazione del nome account è conclusa: `Federico Quadri` è attivo in D1 e l'avatar usa le iniziali `FQ`. Run D1/Worker `33974062257` verde.
- L'avatar account è ancorato in posizione assoluta all'angolo superiore destro. Non partecipa al flusso del layout e il menu `Logout` non sposta né ridimensiona l'agenda.
- Nel calendario, una pressione prolungata di 550 ms sul nome di un giocatore seleziona esclusivamente quel giocatore: restano visibili soltanto i suoi tornei e le sue partite. Il clic breve sulla fascia continua ad aprire normalmente il torneo.
- Dopo l'attivazione del filtro viene soppresso soltanto il clic generato dalla stessa pressione prolungata, evitando aperture accidentali del torneo. Sono gestiti rilascio, annullamento, uscita del puntatore e menu contestuale mobile.
- I nomi giocatore rimangono disponibili anche nel calendario mobile. Cache aggiornate a `v3.js?v=2026090518` e `v3.css?v=2026090506`.
- Pubblicazione verde: Pages `33980021110` e Deploy CourtWatch `33980021643`.

### 2026-09-05 — docs: record header logout and hold fixes (`a3f177d9`)

## Revisione 2026-09-05.152 — intestazione, logout visibile e pressione giocatore robusta
- Rimossa la dicitura `V3` dal titolo del documento e dall'intestazione; rimosso anche il sottotitolo `Agenda-first · motori separati`. Il titolo visibile è ora soltanto `Court Watch`.
- Il menu account è fissato sotto l'avatar con livello superiore all'intera applicazione, sfondo bianco, testo scuro e voce `Logout` esplicita; non occupa spazio nel layout e non interferisce con l'agenda.
- La pressione prolungata di 550 ms per isolare un giocatore è ora collegata sia al nome dentro le fasce torneo sia ai pulsanti con il nome del giocatore sopra il calendario. Piccoli movimenti non annullano più l'azione.
- Durante la pressione viene mostrato un feedback visivo. Al completamento restano selezionati e visibili soltanto tornei, agenda e partite del giocatore scelto; il clic breve conserva il comportamento precedente.
- Cache aggiornate/set: `v3.js?v=2026090519` e `v3.css?v=2026090507`.

### 2026-09-05 — docs: record logout reflow and gender colors (`fd1e3dfc`)

## Revisione 2026-09-05.153 — logout a espansione, colori genere e selezione globale
- Il menu account non è più sovrapposto: resta allineato in alto a destra ma, quando viene aperto, entra nel flusso della testata, ne aumenta l'altezza e spinge verso il basso agenda e resto dell'app.
- I codici evento Tennis Europe femminili che iniziano per `G` (`GS14`, `GD14`, ecc.) usano rosa/viola; quelli maschili che iniziano per `B` (`BS14`, `BD14`, ecc.) usano blu.
- Il comando globale mostra `Deseleziona tutti` soltanto quando sono selezionati tutti i giocatori. Con uno, alcuni o nessun giocatore selezionato mostra `Seleziona tutti` e il clic seleziona l'intero elenco.
- Cache aggiornate a `v3.js?v=2026090520` e `v3.css?v=2026090508`.

### 2026-09-05 — docs: record login regression cause and UI refinements (`7f7cee85`)

## Revisione 2026-09-05.154 — causa regressione login e rifiniture account/eventi
- La precedente incompletezza dopo l'introduzione del login non era causata dalle credenziali: il client aveva sostituito la proiezione universale pre-login con uno snapshot account temporaneamente incompleto; il tag `base` degli asset deviava navigazioni e API relative verso GitHub Pages; cache-buster non aggiornati lasciavano inoltre in esecuzione copie JavaScript precedenti.
- Il menu Logout continua a espandere la testata e spingere l'app verso il basso, ma non usa più alcuna ombra.
...
### 2026-09-05 — Update CourtWatch report revision 155 (`16451129`)
...
### 2026-09-05 — Update CourtWatch report revision 156 (`016adf21`)
...
### 2026-09-05 — Update CourtWatch report revision 157 (`b3d96e18`)
...
### 2026-09-05 — Update CourtWatch report revision 158 (`b6faf308`)
...
### 2026-09-05 — Add CourtWatch release guard and stable baseline (`60742ca8`)
...
### 2026-09-05 — Increase only tournament match-count font size (`93b3aa69`)
...
### 2026-09-05 — Show doubles partner on player match cards (`8aec7c31`)
...
### 2026-09-05 — Extend CourtWatch Shield release protections (`a6de92be`)
...
### 2026-09-05 — Add isolated authenticated CRUD smoke test (`2e33ad87`)
...
### 2026-09-05 — Show official court match number in agenda (`16bcde59`)
...
### 2026-09-05 — Add player year filter and clarify CRUD permission (`741c9173`)
...
### 2026-09-05 — Backfill Tennis Europe court match numbers (`fb6b70f6`)
...
### 2026-09-05 — Fix D1 court match number validation (`b9f95d79`)
...
### 2026-09-05 — Run current authenticated CRUD shield on workflow updates (`9d581b20`)
...
### 2026-09-05 — Keep protected app available during D1 maintenance (`b8ee1924`)
...
### 2026-09-05 — Use official Tennis Europe court match numbers (`f12402b1`)
...
### 2026-09-05 — Use first and last name for player initials (`3e4c6feb`)
...
### 2026-09-05 — Reorder agenda details and emphasize match metadata (`0b01aa87`)
...
### 2026-09-05 — Restore stable agenda rendering and hierarchy (`4636835b`)
...
### 2026-09-05 — Keep opponent last and restore agenda blue (`9811ccf5`)
...
### 2026-09-05 — Place result below opponent in agenda (`a6ce79a2`)

## Revisione 2026-09-05.175 — risultato sotto avversaria
- Il risultato viene mostrato sotto l’avversaria nell’Agenda.

### 2026-09-05 — Force deploy of restored working application (`2537cbc0`)
...
### 2026-09-05 — Place opponent last and restore standard blue (`03e9a399`)
...
### 2026-09-05 — Bump CSS cache for agenda release (`4430715d`)
...
### 2026-09-05 — Finalize agenda colors and result order (`c2c8cb41`)
...
### 2026-09-05 — Use standard blue for agenda match metadata (`b6ccf2de`)
...
### 2026-09-06 — Restore Europe OOP and resolve alternative ITF draws (`f9e8ca55`)
...
### 2026-09-06 — Restore previous ITF T-1 resolution policy (`1613ed4c`)
...
### 2026-09-06 — Add Grimoldi match number and saturate event badges (`08c34cbf`)
...
### 2026-09-06 — Require official Tennis Europe OOP match numbers (`95880fbf`)
...
### 2026-09-06 — Harden official Tennis Europe OOP acquisition (`dd69f6c7`)
...
### 2026-09-06 — Restore Calendar heading and accept official TBA order (`e749ebf6`)
...
### 2026-09-06 — Align player controls with dynamic calendar height (`d5226f80`)
...
### 2026-09-06 — Shrink player column live and scope official OOP rows (`3be09b50`)
...
### 2026-09-06 — Distinguish official unscheduled Europe matches (`1a556048`)
...
### 2026-09-06 — Refine player and tournament match details (`7090a546`)
...
### 2026-09-06 — Fix match detail order and typography (`1f026dbf`)
...
### 2026-09-06 — Remove unrequested filter change (`97b142d0`)
...
### 2026-09-06 — Adjust doubles and result typography (`2ce94b4c`)
...
### 2026-09-06 — Restore singles label in match lists (`4bb70f78`)
...
### 2026-09-06 — Schedule ITF live workflows every 15 minutes through Cloudflare (`cecd8b4a`)
...
### 2026-09-06 — Record successful deployment of 15-minute ITF scheduler (`05578612`)

- Verifica deploy schedulazione: run `34032567890` concluso verde il 6 settembre alle 12:15:54 UTC. Cinque test CI e dry-run Wrangler verdi; log del deploy conferma esplicitamente entrambi i cron `*/10 * * * *` e `*/15 * * * *`, versione Worker `3e1d005c-16ae-4302-882e-e213549a0270`, configurazione token conclusa. Non ancora osservati due tick reali consecutivi del nuovo cron; nessuna garanzia aggiuntiva sui tempi di coda GitHub.

### 2026-09-06 — Fix live scheduling, OOP watchdog, FITP publishing and D1 refresh; stop silent ITF R2 resets (`0676c2de`)

## Revisione 2026-09-06.196 — avvii live, pubblicazione FITP e continuità D1
- Esteso il cron Cloudflare ogni 15 minuti a FITP iscrizioni, Tennis Europe iscrizioni e OOP/risultati, oltre ai tre flussi ITF. Rimossi i cron live GitHub duplicati; il full scan FITP settimanale e le attività giornaliere restano separati. Eliminato il vecchio avvio Europe a seguito FITP per evitare doppie acquisizioni.
- Il catalogo FITP aveva acquisito correttamente i dati, ma falliva durante il rebase del calendario derivato. La pubblicazione conserva i due file sorgente validati, riparte dall'ultimo main e rigenera il calendario sui dati correnti a ogni tentativo, senza rebase di file generati. Il completamento del catalogo avvia esplicitamente il motore iscrizioni.
- D1 ora parte anche dopo il completamento riuscito dei motori tramite workflow_run e viene sorvegliato dal watchdog. Il checkout usa main corrente, le scritture restano serializzate. I cicli ordinari conservano l'indice osservati già presente e aggiornano la proiezione senza reinserire circa 100.000 record; il refresh completo resta disponibile.
- Aggiunta ispezione R2 ITF in sola lettura, con errori di download visibili e conteggi dei tre archivi in current/backup-1/backup-2. Nessuna riparazione alla cieca o modifica CRUD.
...
### 2026-09-06 — Recover retained ITF acceptance snapshots after silent R2 resets (`36d03344`)
...
### 2026-09-06 — Report live workflow failures, stale sources and incomplete ITF archives (`a30e0192`)
...
### 2026-09-06 — Rebuild permanent ITF player and result databases from certified R2 history (`1e9c13dc`)
...
### 2026-09-06 — Fix ITF history archive filename validation (`a4b27728`)
...
### 2026-09-06 — Use portable ITF archive entry count validation (`140d97ea`)
...
### 2026-09-06 — Verify admin boundary and authenticated CRUD without Access token creation (`f6ae4545`)
...
### 2026-09-06 — Configure D1 before authenticated CRUD verification (`f5655afd`)
...
### 2026-09-06 — Make CourtWatch Shield block functional UI regressions (`da9eb904`)
...
### 2026-09-06 — Exercise visible analysis controls in CourtWatch Shield (`2bcc5053`)
...
### 2026-09-06 — Group daily agenda matches by tournament (`b1d3199f`)
...
### 2026-09-06 — Publish grouped agenda with fresh asset versions (`06830db6`)
...
### 2026-09-06 — Refine tournament-divided agenda layout (`ca2ddc97`)
...
### 2026-09-06 — Restore agenda revision on latest diagnostics (`b2172e06`)
...
### 2026-09-06 — Trigger deploy for agenda revision (`6b7df12a`)
...
### 2026-09-06 — Fix profile traversal in CourtWatch Shield (`9cf2297f`)
...
### 2026-09-06 — Document agenda publication fix (`dd594c5d`)
...
### 2026-09-06 — Publish CourtWatch UI revision 207 (`65947f57`)
...
### 2026-09-06 — Fix CourtWatch Shield visible toggle traversal (`9f1e323f`)
...
### 2026-09-06 — Hotfix Cloudflare 1101 on protected app (`a47822e2`)
...
### 2026-09-06 — Add isolated CourtWatch Worker hotfix deploy (`97369946`)
...
### 2026-09-06 — Prevent unhandled Cloudflare 1101 permanently (`9665b15e`)
...
### 2026-09-06 — Align agenda player name with court row (`f87be0bf`)
...
### 2026-09-06 — Keep Access login stable and publish Europe schedule qualifiers (`c87f5ec7`)
...
### 2026-09-06 — Show Europe relative schedule labels in agenda (`c08cd043`)
...
### 2026-09-06 — Refine tournament page links (`bae24fb1`)
...
### 2026-09-07 — Refine Europe relative order labels in agenda (`f4e1ba93`)
...
### 2026-09-07 — Fix Europe OOP parser and D1 ownership parity (`658db121`)

## Revisione 2026-09-07.216 — ripristino OOP Europe e isolamento parità D1
...
### 2026-09-07 — Prevent duplicate Europe agenda D1 queue replacement (`81578d24`)
