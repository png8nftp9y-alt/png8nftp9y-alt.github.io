...
## Aggiornamento 2 settembre 2026 — accesso Admin, liste private e posti giocatore

- Regola generale di prodotto: ogni modifica funzionale o grafica introdotta nell'app si applica per impostazione predefinita a tutti gli utenti presenti e futuri. Una funzione può essere limitata all'Admin soltanto su indicazione esplicita dell'utente; `TCL` e `Altri` sono attualmente le sole eccezioni private dichiarate.
- La pagina Admin è il centro di controllo unico dell'app. Utenti iscritti, liste, capacità, futuri prezzi/pacchetti, profili, motori, D1, R2, backup e audit devono essere gestiti da qui; l'interfaccia utente resta personale.
- `TCL` e `Altri` sono collezioni private del solo account admin. `TCL` viene inizializzata con l'elenco Court Watch corrente; `Altri` parte vuota. L'Admin è illimitato e non consuma posti.
- Ogni futuro utente riceve calendario e agenda vuoti, con un giocatore incluso. Ogni giocatore successivo richiede un posto acquistato tramite `+`; prezzo, durata e pacchetti multipli restano da configurare senza essere codificati nello schema.
- Aggiunta la migrazione `0006_private_player_collections.sql` con collezioni private, associazioni ai profili osservati o agli ID Court Watch, capacità inclusa/acquistata e registro append-only di acquisti, rimborsi e rettifiche.
- Aggiunta la V3 privata `/admin/app`, protetta dalla stessa autenticazione server-side del Control Center. Soltanto questa modalità mostra `+`, `Altri`/`TCL` e ricerca nell'indice universale; la pagina pubblica non espone i comandi admin.
- `Home · Oggi` viene riportato al centro. Un giocatore trovato nell'indice universale entra nella collezione richiesta in stato `pending` finché il suo profilo applicativo non è stato completato dai dati disponibili.
- `/admin/configurazione` mostra utenti iscritti con email, ruolo, stato, data d'iscrizione, liste, giocatori, capacità complessiva e residua. In assenza di registrazioni mostra uno stato vuoto esplicito.
- Correzione UI: i comandi Admin `+` e `Altri`/`TCL` appartengono all'intestazione della colonna destra `Giocatori`, non alla barra dei filtri sopra il calendario. Il markup resta condiviso, ma in modalità Admin i comandi vengono collocati nella card laterale.
- Primo deploy D1 della fondazione Admin: migrazioni applicate correttamente, ma import universale interrotto sul primo file da 2.000 statement per `D1 DB storage operation exceeded timeout` dopo quattro tentativi; Worker non pubblicato e comandi Admin non ancora operativi. Correzione: lotti idempotenti ridotti a 500 statement, mantenendo invariati dati, schema e motori.
