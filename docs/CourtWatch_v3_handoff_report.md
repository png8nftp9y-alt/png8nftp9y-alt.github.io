...
- 12 settembre 2026 — Stabilizzata la UI di classifiche e agenda. L’interfaccia attende la proiezione autorevole e conserva l’ultima classifica Tennis Europe valida durante vuoti temporanei; conserva inoltre i match correnti/futuri mancanti da un singolo aggiornamento finché non sono trascorsi due giorni dalla data prevista. Le classifiche FITP e Tennis Europe restano insieme nella riga informativa grigia sotto il nome. L’intera intestazione del torneo in corso è gialla, mentre le righe delle partite restano bianche.

Revisione documento: **2026-08-31.34**
...
## Aggiornamento 12 settembre 2026 — protezione preventiva definitiva D1

- Lo storico classifiche resta esplicitamente sospeso fino all'OK dell'utente; il perimetro scelto è soltanto giocatori, compagni e avversari presenti in Court Watch.
- Aggiunto un guard preventivo comune ai tre workflow che importano file SQL in courtwatch-app. Prima di qualsiasi chiamata remota blocca file oltre 15 MB, oltre 5.000 istruzioni, oltre 500 cancellazioni o oltre 32 cancellazioni con sottoquery. La classe di regressione che produsse cancellazioni per partita e circa 190,5 milioni di letture viene quindi fermata prima di raggiungere D1.
- Il guard è obbligatorio per rebuild universale, agenda Tennis Europe e classifiche Tennis Europe ed è incluso nella suite sintattica del Worker.
- Reso idempotente anche il refresh settimanale ranking: una pubblicazione già importata non viene più riscritta integralmente; gli UPSERT aggiornano storico e alias soltanto quando i valori effettivi cambiano. Restano invariati acquisizione completa B14/G14/B16/G16, frequenze e API.
- Evidenza precedente alla nuova protezione: run ranking 16 verde con 10.316 posizioni, circa 116.393 righe lette e 61.958 scritte, contro circa 190,5 milioni di letture della regressione eliminata. Il nuovo run deve certificare il guard verde e la seconda esecuzione identica con scritture drasticamente inferiori.
...
## Aggiornamento 13 settembre 2026 — ripristino definitivo del report canonico

- Riparato `docs/CourtWatch_v3_handoff_report.md`, che conteneva una vasta porzione di testo illeggibile dovuta a corruzione di codifica già presente nella cronologia.
- Base di recupero: copia storica leggibile `docs/courtwatch-continuous-report.md`, integrata con gli ultimi aggiornamenti Cloudflare ancora recuperabili dal report danneggiato.
- Il documento canonico torna a essere Markdown UTF-8 leggibile. Il file corrotto resta recuperabile dalla cronologia Git, ma non viene mantenuto nel ramo principale.
- Verifica obbligatoria per modifiche future: il report deve iniziare con il titolo Court Watch, non contenere caratteri di sostituzione Unicode e restare leggibile prima della pubblicazione.
- Punto aperto: certificare su un ciclo senza modifiche che l'impronta D1 produca `rows_written=0`.
- Lavori UI concordati ancora da eseguire: link separati per giocatore e compagno nelle partite di doppio; etichetta rossa `PROGRAMMATI` al posto delle statistiche per i tornei futuri; rimozione della tendina stato torneo.
