...
const eligible=t1Candidates(map.tournaments||[],state.tournaments||{},TODAY,tomorrow,requested).sort((a,b)=>Number(b.endDate<TODAY)-Number(a.endDate<TODAY)||String(a.endDate).localeCompare(String(b.endDate))||String(state.tournaments?.[a.competitionId]?.checkedAt||'').localeCompare(String(state.tournaments?.[b.competitionId]?.checkedAt||''))||String(a.competitionId).localeCompare(String(b.competitionId)));
