...
const completed=matches.filter(x=>x.status==='completed'),scheduled=matches.filter(x=>x.status==='scheduled'),winnerUnresolved=completed.filter(x=>!x.winnerPlayerIds?.length).length,withoutOop=tournaments.filter(x=>(x.dates||[]).length===0).length,sourceFailures=shards.reduce((n,x)=>n+(x.failures||[]).length,0),missingCourtMatchNumbers=matches.filter(x=>!Number.isInteger(Number(x.courtMatchNumber))||Number(x.courtMatchNumber)<=0).length;
const MIN_TOURNAMENTS=Number(process.env.TE_OOP_MIN_TOURNAMENTS||453);
const result={version:'te-oop-historical-v1',generatedAt:new Date().toISOString(),status:shards.length===EXPECTED_SHARDS&&missing.length===0&&tournaments.length>=MIN_TOURNAMENTS&&conflicts.length===0&&sourceFailures===0&&winnerUnresolved===0&&missingCourtMatchNumbers===0?'green':'red',sourceRunId:process.env.TE_OOP_SOURCE_RUN_ID||null,minimumTournamentCount:MIN_TOURNAMENTS,counts:{shards:shards.length,tournaments:tournaments.length,withoutOop,matches:matches.length,completed:completed.length,scheduled:scheduled.length,singles:matches.filter(x=>x.players.length===2).length,doubles:matches.filter(x=>x.players.length===4).length,uniqueNominalIdentities:identities.length,winnerUnresolved,sourceFailures,missingCourtMatchNumbers,conflicts:conflicts.length},missingShards:missing,conflicts,tournaments,matches,identities};
