...
const probes=[];for(const candidate of candidates.values()){if(candidate.base){probes.push(candidate);continue}try{const page=await request('GET',candidate.url,{cookie});if(page.status===200&&/\/tournament\/[^/]+\/matches/i.test(candidate.url)){await fs.mkdir('dist/v3/tennis-europe-oop-html',{recursive:true});await fs.writeFile('dist/v3/tennis-europe-oop-html/'+id+'.html',page.text)}probes.push({...candidate,status:page.status,location:page.location,evidence:evidence(page.text)})}catch(error){probes.push({...candidate,error:String(error)})}}
