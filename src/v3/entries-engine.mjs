...
const itfEntries=[...(itfHistory.entries||[]),...(itf.entries||[])].map(r=>{const official=itfTournamentById.get(String(r.competitionId||'').toUpperCase())||{};return entry({...r,circuit:'itf',tournamentName:r.tournamentName||official.tournamentName,location:official.location||r.location,venueName:official.venue||official.venueName||r.venueName||''})});
