...
const diagnosticsItems=[
 ...
 light(teSystem.status==='green'?'green':teSystem.status==='red'?'red':'yellow','Tennis Europe',`${(te.entries||[]).filter(e=>e.circuit==='tennis-europe').length} presenze · T−1 ${teSystem.checks?.confirmed||0} confermati/${teSystem.checks?.rejected||0} esclusi · mappa ${teSystem.checks?.mapTournaments||0} · continuità ${teSystem.checks?.continuityCycles||0} cicli`,true),
