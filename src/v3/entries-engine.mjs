...
const entries=[...fitpEntries,...teEntries,...itfEntries].map(e=>e.circuit?e:entry(e)).filter(e=>e.playerId&&validDate(e.endDate));
...
const diagnosticsItems=[
 ...
 light(itf.status==='itf_acceptance_complete'?'green':'yellow','ITF',`${itfEntries.length} entry · live ${(itf.entries||[]).length} · storico ${(itfHistory.entries||[]).length} · stato ${itf.status||'n/d'}`,false),
