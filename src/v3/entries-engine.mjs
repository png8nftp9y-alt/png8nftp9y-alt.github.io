...
const entries=[...fitpEntries,...teEntries,...(itf.entries||[])].map(e=>e.circuit?e:entry(e)).filter(e=>e.playerId&&validDate(e.endDate));
...
const diagnosticsItems=[
 ...
 light(teSystem.status==='green'?'green':teSystem.status==='red'?'red':'yellow','Tennis Europe',`${teEntries.length} entry · T−1 ${teSystem.checks?.confirmed||0} confermati/${teSystem.checks?.rejected||0} esclusi · mappa ${teSystem.checks?.mapTournaments||0} · continuità ${teSystem.checks?.continuityCycles||0} cicli`,true),
