...
const audit={version:2,generatedAt:new Date().toISOString(),status:remainingRetries?'itf_section_retry_incomplete':'itf_section_retry_complete',patches:patches.length,originalRetries,resolvedRetries,remainingRetries,summary:{populated:sections.filter(x=>x.status==='populated').length,publishedEmpty:sections.filter(x=>x.status==='published_empty').length,missingOrUnreadable:sections.filter(x=>x.status==='missing_or_unreadable').length},sections};
