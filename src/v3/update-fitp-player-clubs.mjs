...
for(const p of data.players||[]){
 ...
 try{
  ...
  const before=p.club||'';
  if(club) p.club=club;
  recovered.push({playerId:p.id,playerName:p.name,membershipCard:p.membershipCard,status:res.status,rawClub:raw,club:p.club||'',changed:before!==p.club,ranking:res.json?.player?.ranking||res.json?.player?.Ranking||''});
...
await writeJson('dist/v3/fitp_player_clubs_recovered.json',{generatedAt:NOW,playersInput:(data.players||[]).length,profilesChecked:recovered.length,clubsRecovered:recovered.filter(r=>r.club).length,changed:recovered.filter(r=>r.changed).length,results:recovered,errors});
console.log(JSON.stringify({generatedAt:NOW,clubsRecovered:recovered.filter(r=>r.club).length,changed:recovered.filter(r=>r.changed).length,errors},null,2));
