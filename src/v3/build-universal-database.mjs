...
for(const row of entriesDoc.tournamentEntries||[]){
 const circuit=normalizeCircuit(row.circuit),sourceId=String(row.competitionId||row.tournamentName||''),tournamentId=stableId('tournament',circuit,sourceId),effectiveStartDate=circuit==='itf'?(row.qualificationStartDate||row.officialStartDate||row.startDate):row.startDate;
 const tournament=compact({id:tournamentId,circuit,sourceTournamentId:sourceId,name:row.tournamentName||'',location:row.location||'',surface:row.surface||'',environment:row.environment||row.indoorOutdoor||'',startDate:ensureIsoDate(effectiveStartDate,'tournament.startDate'),endDate:ensureIsoDate(row.endDate,'tournament.endDate'),officialStartDate:ensureIsoDate(row.officialStartDate||effectiveStartDate,'tournament.officialStartDate'),status:row.status||'known',source:sourceRef(circuit,sourceId,row.sourceUrl,row.lastSeen||entriesDoc.generatedAt)});
