...
export async function tournamentEvents(t){const r=await request(`${API}/GetEventFilters?tournamentKey=${encodeURIComponent(t.competitionId.toLowerCase())}`);if(r.incapsula)throw new Error('GetEventFilters_incapsula_challenge');if(!r.ok||!r.json)throw new Error(`GetEventFilters_${r.status||r.responseKind||'non_json'}`);return eventCombinations(r.json)}
export async function drawsheet(c){const q=new URLSearchParams({tournamentId:String(c.tournamentId),tourType:String(c.tourType||'N'),weekNumber:String(c.weekNumber||0),playerTypeCode:String(c.playerTypeCode),matchTypeCode:String(c.matchTypeCode),eventClassificationCode:String(c.eventClassificationCode),drawsheetStructureCode:String(c.drawsheetStructureCode)});const r=await request(`${API}/GetDrawsheet?${q}`);if(r.incapsula)throw new Error('GetDrawsheet_incapsula_challenge');if(!r.ok||!r.json)throw new Error(`GetDrawsheet_${r.status||r.responseKind||'non_json'}`);return r.json}
