...
const teTournaments=await fetchJson(TENNIS_EUROPE_REF,'dist/v3/source_tennis_europe_tournaments_sharded.json').catch(()=>fetchJson(TENNIS_EUROPE_REF,'dist/v3/source_tennis_europe_tournaments.json')).catch(()=>({}));
const itf=await fetchJson(ITF_REF,'dist/v3/source_itf_entries.json');
const itfTournaments=await fetchJson(ITF_REF,'dist/v3/source_itf_tournaments.json').catch(()=>({}));
...
const teEntries=(te.entries||[]).map(r=>{const official=teTournamentById.get(String(r.competitionId||'').toUpperCase())||{};return entry({...r,circuit:'tennis-europe',tournamentName:official.tournamentName||r.tournamentName,location:official.location||r.location,venueName:official.venueName||r.venueName||''})}).filter(e=>e.playerId&&e.startDate&&validDate(e.endDate));
const fitpEntries=(fitp.entries||[]).map(r=>{const official=fitpTournamentById.get(String(r.competitionId||'').toUpperCase())||{};return entry({...r,circuit:'fitp',venueName:r.venueName||official.club||'',address:r.address||official.address||''})}).filter(e=>e.playerId&&validDate(e.endDate)&&!externalTournamentInFitp(e));
const fitpExternalExcluded=(fitp.entries||[]).length-fitpEntries.length;
const itfEntries=(itf.entries||[]).map(r=>{const official=itfTournamentById.get(String(r.competitionId||'').toUpperCase())||{};return entry({...r,circuit:'itf',tournamentName:r.tournamentName||official.tournamentName,location:official.location||r.location,venueName:official.venue||official.venueName||r.venueName||''})}).filter(e=>e.playerId&&validDate(e.endDate));
