...
function makeSpecs(){
  ...
  for(const regionId of REGION_IDS){
    ...
    for(const days of [7,14,21,31,62,124])for(let s=new Date(start);s<=limit;s=addDays(s,Math.max(1,days-1))){const e=new Date(Math.min(limit.getTime(),addDays(s,days-1).getTime()));out.push({mode:`region_${regionId}_${days}d`,start:new Date(s),end:e,id_regione:regionId})}
    for(const stateId of STATE_IDS)for(const days of [7,14,31])for(let s=new Date(start);s<=limit;s=addDays(s,Math.max(1,days-1))){const e=new Date(Math.min(limit.getTime(),addDays(s,days-1).getTime()));out.push({mode:`region_${regionId}_state_${stateId}_${days}d`,start:new Date(s),end:e,id_regione:regionId,id_stato:stateId})}
  }
  // Additional official id_provincia sweep. This is deliberately structural, not keyword-based, and is meant to catch tournaments hidden by broad region/list caps.
  for(const provinceId of PROVINCE_IDS){
    out.push({mode:`province_${provinceId}_all`,start:null,end:null,id_provincia:provinceId});
    for(const stateId of STATE_IDS)out.push({mode:`province_${provinceId}_state_${stateId}`,start:null,end:null,id_provincia:provinceId,id_stato:stateId});
    for(const days of [14,31,62,124])for(let s=new Date(start);s<=limit;s=addDays(s,Math.max(1,days-1))){const e=new Date(Math.min(limit.getTime(),addDays(s,days-1).getTime()));out.push({mode:`province_${provinceId}_${days}d`,start:new Date(s),end:e,id_provincia:provinceId})}
  }
  ...
  for(const eta of ['U08','U09','U10','U11','U12','U13','U14','U15','U16','U17','U18','NOF','NOR'])out.push({mode:'cat_eta_'+eta,start:null,end:null,categoria_eta:eta});
  return out;
}
...
const out={version:'cw-v3-agenda-first',generatedAt:NOW,status:errors.length?'fitp_individual_tournament_discovery_province_sweep_with_errors':'fitp_individual_tournament_discovery_province_sweep_complete',source:'FITP individual tournaments only: official P.U.C. list, id_disciplina=4332 tennis, cod_fonte=1 Tornei FITP. Adds region+state+short-date and province+date decomposition to reduce broad-list caps. Tennis Europe/TE tournaments excluded. No player keywords, no team championships, no draws, no OOP, no results.',coverageFrom:FROM,coverageUntil:addDays(new Date(TODAY+'T00:00:00Z'),HORIZON_DAYS).toISOString().slice(0,10),queries:queries.length,tournamentsFound:tournaments.length,bySource:{'TORNEI FITP':tournaments.length},byStatus,quality:{specs:specs.length,failedQueries:errors.length,emptyQueries:queries.filter(q=>q.rows===0).length,saturatedQueries,uniqueAccepted:tournaments.length,excluded:excludedMap.size,usesPlayerKeywords:false,usesDraws:false,usesOrderOfPlay:false,usesResults:false,individualOnly:true,teamChampionshipsIncluded:false,tennisEuropeExcluded:true,finalStructuredSweep:true,regionStateDateSweep:true,provinceSweep:true,regionIds:REGION_IDS,stateIds:STATE_IDS,provinceIds:PROVINCE_IDS,byMode},tournaments,errors};
...
if(tournaments.length<5000||errors.length>200)process.exitCode=1;
