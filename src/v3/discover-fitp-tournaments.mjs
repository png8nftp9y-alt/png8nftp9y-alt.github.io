...
function addWindowSpecs(out,start,limit,daysList,stepFn,extra={}){for(const days of daysList)for(let s=new Date(start);s<=limit;s=addDays(s,stepFn(days))){const e=new Date(Math.min(limit.getTime(),addDays(s,days-1).getTime()));out.push({...extra,mode:[extra.modePrefix,`${days}d`].filter(Boolean).join('_')||`${days}d`,start:new Date(s),end:e})}}
function makeSpecs(){
  ...
  addWindowSpecs(out,start,limit,[2,3,5,7,10,14,21,31,62],d=>Math.max(1,d-1));
  for(const regionId of REGION_IDS){
    ...
    for(const stateId of STATE_IDS){
      for(const days of [7,14,31])for(let s=new Date(start);s<=limit;s=addDays(s,Math.max(1,days-1))){const e=new Date(Math.min(limit.getTime(),addDays(s,days-1).getTime()));out.push({mode:`region_${regionId}_state_${stateId}_${days}d`,start:new Date(s),end:e,id_regione:regionId,id_stato:stateId})}
    }
  }
  out.push({mode:'no_date_all',start:null,end:null});
  // Freetext remains only as secondary fallback for public tournament descriptors; no player names/cards are used.
...
const out={version:'cw-v3-agenda-first',generatedAt:NOW,status:errors.length?'fitp_individual_tournament_discovery_final_with_errors':'fitp_individual_tournament_discovery_final_complete',source:'FITP individual tournaments only: official P.U.C. list, id_disciplina=4332 tennis, cod_fonte=1 Tornei FITP. Final structured sweep adds region+state+short-date decomposition to reduce saturated broad-list caps. Tennis Europe/TE tournaments excluded. No player keywords, no team championships, no draws, no OOP, no results.',coverageFrom:FROM,coverageUntil:addDays(new Date(TODAY+'T00:00:00Z'),HORIZON_DAYS).toISOString().slice(0,10),queries:queries.length,tournamentsFound:tournaments.length,bySource:{'TORNEI FITP':tournaments.length},byStatus,quality:{specs:specs.length,failedQueries:errors.length,emptyQueries:queries.filter(q=>q.rows===0).length,saturatedQueries,uniqueAccepted:tournaments.length,excluded:excludedMap.size,usesPlayerKeywords:false,usesDraws:false,usesOrderOfPlay:false,usesResults:false,individualOnly:true,teamChampionshipsIncluded:false,tennisEuropeExcluded:true,finalStructuredSweep:true,regionStateDateSweep:true,regionIds:REGION_IDS,stateIds:STATE_IDS,byMode},tournaments,errors};
const audit={...out,tournaments:undefined,queries,excluded:[...excludedMap.values()].slice(0,1200),sample:tournaments.slice(0,200)};
...
if(tournaments.length<4500||errors.length>150)process.exitCode=1;
