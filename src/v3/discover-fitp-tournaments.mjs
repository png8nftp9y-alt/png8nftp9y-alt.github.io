...
const out={version:'cw-v3-agenda-first',generatedAt:NOW,status:errors.length?'fitp_individual_tournament_discovery_structured_region_with_errors':'fitp_individual_tournament_discovery_structured_region_complete',source:'FITP individual tournaments only: official P.U.C. list, id_disciplina=4332 tennis, cod_fonte=1 Tornei FITP. Adds official structured id_regione sweep + region/date/state decomposition to bypass broad-list caps. Tennis Europe/TE tournaments are excluded from FITP stream. No player keywords, no team championships, no draws, no OOP, no results.',coverageFrom:FROM,coverageUntil:addDays(new Date(TODAY+'T00:00:00Z'),HORIZON_DAYS).toISOString().slice(0,10),queries:queries.length,tournamentsFound:tournaments.length,bySource:{'TORNEI FITP':tournaments.length},byStatus,quality:{specs:specs.length,failedQueries:errors.length,emptyQueries:queries.filter(q=>q.rows===0).length,saturatedQueries,uniqueAccepted:tournaments.length,excluded:excludedMap.size,usesPlayerKeywords:false,usesDraws:false,usesOrderOfPlay:false,usesResults:false,individualOnly:true,teamChampionshipsIncluded:false,tennisEuropeExcluded:true,structuredRegionSweep:true,regionIds:REGION_IDS,byMode},tournaments,errors};
