...
function makeSpecs(){
  ...
  // Category/sex filters: not player-keywords; they split official PUC universe to bypass broad-list caps.
  ...
  for(const sesso of ['M','F'])out.push({mode:'sesso_'+sesso,start:null,end:null,sesso});
  for(const tipo of ['Maschile','Femminile','Doppio M.','Doppio F.'])out.push({mode:'tipo_'+tipo,start:null,end:null,tipo_competizione:tipo});
  return out;
}
function payload(spec,skip){const p=basePayload();p.rowstoskip=skip;if(spec.start){p.data_inizio=it(spec.start);p.data_fine=it(spec.end)}for(const k of ['freetext','categoria_eta','sesso','tipo_competizione'])if(spec[k])p[k]=spec[k];return p}
...
const out={version:'cw-v3-agenda-first',generatedAt:NOW,status:errors.length?'fitp_individual_tournament_discovery_broadened_with_errors':'fitp_individual_tournament_discovery_broadened_complete',source:'FITP individual tournaments only: official P.U.C. list, id_disciplina=4332 tennis, cod_fonte=1 Tornei FITP. Broadened multi-axis sweep: dense date windows, region/competition/category/type public filters, no player keywords, no team championships, no draws, no OOP, no results.',coverageFrom:FROM,coverageUntil:addDays(new Date(TODAY+'T00:00:00Z'),HORIZON_DAYS).toISOString().slice(0,10),queries:queries.length,tournamentsFound:tournaments.length,bySource:{'TORNEI FITP':tournaments.length},byStatus,quality:{specs:specs.length,failedQueries:errors.length,emptyQueries:queries.filter(q=>q.rows===0).length,uniqueAccepted:tournaments.length,excluded:excludedMap.size,usesPlayerKeywords:false,usesDraws:false,usesOrderOfPlay:false,usesResults:false,individualOnly:true,teamChampionshipsIncluded:false,broadenedSweep:true,byMode},tournaments,errors};
...
if(tournaments.length<900||errors.length>150)process.exitCode=1;
