...
const priority=e=>({MD:0,Q:1,A:2}[e.acceptanceCode]??3),position=e=>Number.isFinite(Number(e.acceptancePosition))?Number(e.acceptancePosition):Number.MAX_SAFE_INTEGER;
const out={version:'cw-v3-agenda-first',generatedAt:NOW,status:errors.length?'tennis_europe_sharded_acceptance_blocked':'tennis_europe_acceptance_complete',source:'Merged sharded acceptance output classified from official row colors (green MD, yellow Q, dark orange A), with section fallback.',coverageFrom:FROM,shards,tournamentsChecked:shards.reduce((a,s)=>a+(s.tournamentsChecked||0),0),entriesFound:dedup.length,byPlayer,byAcceptance,entries:dedup.sort((a,b)=>(a.startDate||'9999').localeCompare(b.startDate||'9999')||String(a.competitionId).localeCompare(String(b.competitionId))||priority(a)-priority(b)||position(a)-position(b)||a.playerName.localeCompare(b.playerName)),errors};
