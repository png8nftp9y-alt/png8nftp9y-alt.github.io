...
publish(){
  work="$(mktemp -d)"
  jq -e --argjson minTournaments "$MIN_TOURNAMENTS" --argjson minMatches "$MIN_MATCHES" '.status=="green" and .counts.tournaments >= $minTournaments and .counts.matches >= $minMatches and .counts.winnerUnresolved==0 and .counts.conflicts==0' "$ARCHIVE" >/dev/null
