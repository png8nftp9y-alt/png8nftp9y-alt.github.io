...
const TARGET=process.env.FITP_PLAYER_NAME||'GARIBALDI MATTIA';
const TERMS=(process.env.FITP_PLAYER_TERMS||'GARIBALDI MATTIA,MATTIA GARIBALDI,GARIBALDI').split(',').map(s=>s.trim()).filter(Boolean);
...
await writeJson('dist/v3/fitp_player_probe_mattia_garibaldi.json',out);
