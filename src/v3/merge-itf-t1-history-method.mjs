...
for(const section of eventInventory){
 ...
 family.events.push({
  ...
  error:terminalAlternative?'unused_alternative_structure':(doc?.status==='retry'?(doc.error||'draw_retry_required'):(!populated?'draw_not_acquired':null)),
  failureType:terminalAlternative?'unused_alternative_structure':(doc?.failureType||(!populated?'not_published_or_incomplete':null)),
...
const retryDocs=tournamentDocs.filter(doc=>doc.status!=='complete'&&!eventCache[doc.event]?.terminalAlternative),technicalDocs=retryDocs.filter(doc=>doc.failureType==='technical_error'),publicationDocs=retryDocs.filter(doc=>doc.failureType!=='technical_error'),inventoryTechnical=Boolean(inventory.inventoryError),eventFailure=[inventory.inventoryError,...technicalDocs.map(doc=>doc.error||'technical_error')].filter(Boolean).join(' | ')||null;
