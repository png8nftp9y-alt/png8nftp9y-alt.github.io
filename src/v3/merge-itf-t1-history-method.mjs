...
const retryDocs=tournamentDocs.filter(doc=>doc.status!=='complete'&&!eventCache[doc.event]?.terminalAlternative),technicalDocs=retryDocs.filter(doc=>doc.failureType==='technical_error'),publicationDocs=retryDocs.filter(doc=>doc.failureType!=='technical_error'),missingArtifactSections=eventInventory.filter(section=>!eventCache[section.event]?.populated&&!eventCache[section.event]?.terminalAlternative&&!byEvent.has(section.event)),inventoryTechnical=Boolean(inventory.inventoryError),technicalErrorSections=technicalDocs.length+missingArtifactSections.length+Number(inventoryTechnical),eventFailure=[inventory.inventoryError,...technicalDocs.map(doc=>doc.error||'technical_error'),...missingArtifactSections.map(section=>section.event+':draw_artifact_missing')].filter(Boolean).join(' | ')||null;
