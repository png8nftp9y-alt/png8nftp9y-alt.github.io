...
const retryDocs=tournamentDocs.filter(doc=>doc.status!=='complete'),technicalDocs=retryDocs.filter(doc=>doc.failureType==='technical_error'),publicationDocs=retryDocs.filter(doc=>doc.failureType!=='technical_error'),inventoryTechnical=Boolean(inventory.inventoryError),eventFailure=[inventory.inventoryError,...technicalDocs.map(doc=>doc.error||'technical_error')].filter(Boolean).join(' | ')||null;
