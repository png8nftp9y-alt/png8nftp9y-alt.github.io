...
const retryDocs=docs.filter(doc=>doc.status!=='complete'),technicalDocs=retryDocs.filter(doc=>doc.failureType==='technical_error'),publicationDocs=retryDocs.filter(doc=>doc.failureType!=='technical_error'),eventFailure=technicalDocs.length?technicalDocs.map(doc=>doc.error||'technical_error').join(' | '):null;
...
await writeJson('dist/v3/source_itf_draw_audit.json',{version:11,generatedAt:NOW,status:certificationStatus,summary:{mode:'live_t_minus_1_historical_acquisition',tournamentsChecked:1,completeTournaments:complete?1:0,pendingTournaments:complete?0:1,declaredSections:eventInventory.length,requestedSections:docs.length,acquiredSections,missingSections,newSectionsCached,retrySections:retryDocs.length,technicalErrorSections:technicalDocs.length,notPublishedOrIncompleteSections:publicationDocs.length,confirmedInDraw:found.size,removedByReliableDrawAbsence:removed,globalCompleteTournaments:values.filter(item=>item?.decision==='complete').length,globalPendingTotal:globalPending.length,globalPendingTechnical:globalPendingTechnical.length,globalPendingPublication:globalPendingPublication.length,technicalPendingCompetitionIds:globalPendingTechnical.map(item=>item.competitionId),publicationPendingCompetitionIds:globalPendingPublication.map(item=>item.competitionId)},audit:[{competitionId:id,tournamentName:tournament.tournamentName||inventory.tournamentName,decision:complete?'complete':'pending',playersFound:[...found.keys()],declaredSections:eventInventory.length,acquiredSections,missingSections,sections:eventInventory.map(section=>({event:section.event,family:section.family,acquired:Boolean(eventCache[section.event]?.populated)})),families:[...families].map(([family,value])=>({family,...value})),eventFailure}]});
console.log(JSON.stringify({competitionId:id,declaredSections:eventInventory.length,requestedSections:docs.length,acquiredSections,missingSections,retrySections:retryDocs.length,technicalErrorSections:technicalDocs.length,notPublishedOrIncompleteSections:publicationDocs.length,decision:complete?'complete':'pending',globalPendingTechnical:globalPendingTechnical.length,globalPendingPublication:globalPendingPublication.length},null,2));
if(String(process.env.ITF_FAIL_ON_TECHNICAL||'')==='1'&&technicalDocs.length)process.exitCode=2;
