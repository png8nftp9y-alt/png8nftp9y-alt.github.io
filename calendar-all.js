...
renderAgenda=function(){
  ...
  $('agendaDate').textContent=fmt(state.agenda)+' · storico dal 18/12/2025';
  $('dailyAgenda').innerHTML=matches.length?`<div class="agendaList">${matches.map(m=>{const x=matchMeta(m);const source=circuit(m);const done=m.result||m.status==='completed'||m.status==='cancelled';return`<article class="agendaItem ${source}" data-player="${esc(m.playerId)}"><time>${esc(m.time||'—')}</time><div><div class="agendaTitle"><strong>${esc(m.playerName)}</strong><span class="type ${x.isDouble?'double':''}">${x.isDouble?'Doppio':'Singolare'}</span><span class="type ${source}">${source==='itf'?'ITF':source==='tennis-europe'?'Tennis Europe':'FITP'}</span>${x.conditional?'<span class="conditional">Condizionata</span>':''}</div><p>${esc(m.tournamentName)}${m.court?' · Campo '+esc(m.court):''}</p><p class="versus">${m.partner?'con '+esc(m.partner)+' · ':''}${opponentHtml(m,x)}</p>${x.condition?`<p class="condition">${esc(x.condition)}</p>`:''}${m.result?`<p class="result">Risultato: ${esc(m.result)}</p>`:''}${m.cancellationReason?`<p class="condition">${esc(m.cancellationReason)}</p>`:''}</div><span class="badge ${done?'finished':'upcoming'}">${done?'Storico':'Partita'}</span></article>`}).join('')}</div>`:'<div class="empty">Nessuna partita pubblicata per questa giornata. L’agenda resta navigabile a ritroso dal 18/12/2025.</div>';
