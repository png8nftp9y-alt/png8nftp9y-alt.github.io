...
renderAgenda=function(){
  ...
  if(key==='2026-08-05'){
    matches=matches.filter(m=>String(m.orderOfPlayFile||'').includes('20260805')||String(m.orderOfPlayLine||'').includes('2026-08-05'));
  }
  ...
  $('dailyAgenda').innerHTML=matches.length?`<div class="agendaList agendaSideBySide">${matches.map(m=>{const x=matchMeta(m);const source=circuit(m);const done=m.result||m.status==='completed'||m.status==='cancelled';return`<article class="agendaItem ${source}" data-player="${esc(m.playerId)}"><time class="${m.time?'officialTime':'missingTime'}">${esc(m.time||'orario da pubblicare')}</time><div><div class="agendaTitle"><strong>${esc(m.playerName)}</strong><span class="type ${x.isDouble?'double':''}">${x.isDouble?'Doppio':'Singolare'}</span><span class="type ${source}">${source==='itf'?'ITF':source==='tennis-europe'?'Tennis Europe':'FITP'}</span></div><p>${esc(m.tournamentName)}${m.court?' · Campo '+esc(m.court):''}</p><p class="versus">${m.partner?'con '+esc(m.partner)+' · ':''}${opponentHtml(m,x)}</p>${m.result?`<p class="result">Risultato: ${esc(m.result)}</p>`:''}${m.cancellationReason?`<p class="condition mutedCondition">${esc(m.cancellationReason)}</p>`:''}</div><span class="badge ${done?'finished':'upcoming'}">${done?'Storico':'Partita'}</span></article>`}).join('')}</div>`:'<div class="empty">Nessuna partita pubblicata per questa giornata. L’agenda resta navigabile a ritroso dal 18/12/2025.</div>';
