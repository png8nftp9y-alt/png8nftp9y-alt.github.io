...
assert.ok(hasApiSource('normalized_name IN') && hasApiSource("nameKey(row.normalized_name)===playerNameKey") && !hasApiSource('participantTokens'), 'opponent participants must resolve frozen rankings through indexed exact D1 lookups');
assert.ok(!hasSource('Caricamento storico…') && hasSource('if(prepared){') && hasSource('renderOpponentHistory(prepared)'), 'prepared opponent profiles must render immediately without a visible loading state');
assert.ok(hasSource('ACTIVE_OPPONENT_CACHE') && hasSource('savedActiveOpponent(requestKey)') && hasSource('saveActiveOpponent(requestKey,data)'), 'the active opponent profile must survive a full page refresh without becoming empty');
...
assert.ok(hasApiSource('stableProfileId?await optionalRows') && hasApiSource('profileRows=profileById.length?profileById:await optionalRows') && hasApiSource('profile,tournaments:') && hasSource('profileRanking.textContent'), 'opponent header must reject draw-local numeric IDs and use dated ranking history by exact name');
