...
function hydrateRelativeSchedules(matches){const groups=new Map();for(const match of matches){const key=[match.competitionId||'',isoDate(match.date),normalize(match.court||'')].join('|'),list=groups.get(key)||[];list.push(match);groups.set(key,list)}for(const list of groups.values()){list.sort((a,b)=>positiveMatchNumber(a.courtMatchNumber)-positiveMatchNumber(b.courtMatchNumber));const firstTimed=list.find(match=>match.time)||null;let anchor=null;for(const match of list){const current=positiveMatchNumber(match.courtMatchNumber);match.overallMatchNumber=current||null;match.overallFromTime=firstTimed?.time||'';if(match.followedBy||match.scheduleQualifier==='Followed by'){const base=positiveMatchNumber(anchor?.courtMatchNumber);match.relativeMatchNumber=current&&base&&current>=base?current-base+1:null;match.relativeFromTime=anchor?.time||''}if(match.time)anchor={courtMatchNumber:match.courtMatchNumber,time:match.time}}}}
hydrateRelativeSchedules(matchMap.values());
...
for(const match of matchMap.values())for(const player of match.players||[]){const key=normalize(player.name),owners=ownersByName.get(key)||[];if(!owners.length)continue;if((sourceIdentities.get(key)?.size||0)>1){ambiguous.push({type:'source_identity_collision',key});continue}for(const owner of owners){
 ...
 const row={id:'te-app:'+owner.id+'|'+match.id,playerId:owner.id,playerName:owner.name,sourcePlayerName:player.name,sourceNationality:player.nationality||'',matchId:match.id,competitionId:match.competitionId,tournamentName:tournament.tournamentName||tournament.name||'',location:tournament.location||'',date:isoDate(match.date),time:match.time||'',notBefore:Boolean(match.notBefore),afterRest:Boolean(match.afterRest),followedBy:Boolean(match.followedBy),scheduleQualifier:match.scheduleQualifier||'',relativeMatchNumber:match.relativeMatchNumber||null,relativeFromTime:match.relativeFromTime||'',overallMatchNumber:match.overallMatchNumber||null,overallFromTime:match.overallFromTime||'',court:match.court||'',courtMatchNumber:match.courtMatchNumber||null,event:match.event||'',draw:match.event||'',round:match.round||'',status:match.status,score:displayScore,result:completed?displayScore:'',winnerPlayerIds:match.winnerPlayerIds||[],winnerTeamIndex:winnerTeam,advances:completed&&winnerTeam!==null?winnerTeam===teamIndex:null,teamIndex,matchType:(match.teams||[]).some(team=>(team||[]).length>1)?'doubles':'singles',partner,partnerNationalities:partners.map(x=>x.nationality||''),opponent:opponents.join(' / '),opponentOptions:opponents,opponentNationalities:opponentPlayers.map(x=>x.nationality||''),sourceUrl:match.sourceUrl,drawUrl:match.drawUrl||'',circuit:'tennis-europe',linkMethod:'exact_normalized_full_name'};
