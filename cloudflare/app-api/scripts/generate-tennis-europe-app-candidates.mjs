...
for(const match of matchMap.values())for(const player of match.players||[]){const key=normalize(player.name),owners=ownersByName.get(key)||[];if(!owners.length)continue;if((sourceIdentities.get(key)?.size||0)>1){ambiguous.push({type:'source_identity_collision',key});continue}for(const owner of owners){
 ...
 const partnerPlayers=(match.players||[]).filter(x=>teamIds.has(String(x.id))&&String(x.id)!==playerId).map(x=>({name:x.name,nationality:x.nationality||''}));
 const opponentPlayers=(match.players||[]).filter(x=>!teamIds.has(String(x.id))).map(x=>({name:x.name,nationality:x.nationality||''}));
 const partner=partnerPlayers.map(x=>x.name).join(' / '),opponents=opponentPlayers.map(x=>x.name);
