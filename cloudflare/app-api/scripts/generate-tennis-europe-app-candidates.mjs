...
for(const match of matchMap.values())for(const player of match.players||[]){const key=normalize(player.name),owners=ownersByName.get(key)||[];if(!owners.length)continue;if((sourceIdentities.get(key)?.size||0)>1){ambiguous.push({type:'source_identity_collision',key});continue}for(const owner of owners){const tournament=tournamentMap.get(match.competitionId)||{},row={playerId:owner.id,playerName:owner.name,sourcePlayerName:player.name,sourceNationality:player.nationality||'',matchId:match.id,competitionId:match.competitionId,tournamentName:tournament.tournamentName||tournament.name||'',location:tournament.location||'',date:isoDate(match.date),time:match.time||'',court:match.court||'',event:match.event||'',round:match.round||'',status:match.status,score:match.score||'',winnerPlayerIds:match.winnerPlayerIds||[],sourceUrl:match.sourceUrl,circuit:'tennis-europe',linkMethod:'exact_normalized_full_name'};candidates.set(owner.id+'|'+match.id,row)}}
...
const result={status:'green',counts:{monitoredEuropePlayers:monitored.length,uniqueMatches,playerMatchOccurrences:rows.length,completed,scheduled,ambiguous:0},publishedToAgenda:false};
await fs.writeFile('seed-tennis-europe-oop/app-match-candidates-manifest.json',JSON.stringify(result,null,2)+'\n');
if(uniqueMatches!==147||rows.length!==149)throw new Error(`Candidate parity changed: uniqueMatches=${uniqueMatches}, occurrences=${rows.length}`);
