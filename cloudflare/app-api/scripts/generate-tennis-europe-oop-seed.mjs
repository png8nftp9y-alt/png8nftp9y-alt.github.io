...
for(let i=0;i<importMatches.length;i++){
 ...
 s.push(`INSERT OR REPLACE INTO schedules(id,tournament_id,match_id,circuit,local_date,payload) VALUES(${esc(`oop:${m.id}`)},${esc(tid)},${esc(m.id)},'tennis-europe',${esc(date)},${payload({date,time:m.time||'',court:m.court||'',courtMatchNumber:m.courtMatchNumber||null,event:m.event||'',round:m.round||'',status:m.status,sourceUrl:m.sourceUrl})});`);
