...
for(let i=0;i<importMatches.length;i++){
 ...
 if(incremental){s.push(`DELETE FROM app_match_candidates WHERE match_id=${esc(m.id)};`);s.push(`DELETE FROM match_participants WHERE match_id=${esc(m.id)};`);s.push(`DELETE FROM results WHERE match_id=${esc(m.id)};`);s.push(`DELETE FROM schedules WHERE match_id=${esc(m.id)};`)}
 s.push(`INSERT INTO matches(id,tournament_id,circuit,played_date,payload) VALUES(${esc(m.id)},${esc(tid)},'tennis-europe',${esc(date)},${payload(m)}) ON CONFLICT(id) DO UPDATE SET tournament_id=excluded.tournament_id,circuit=excluded.circuit,played_date=excluded.played_date,payload=excluded.payload;`);
 s.push(`INSERT OR REPLACE INTO schedules(id,tournament_id,match_id,circuit,local_date,payload) VALUES(${esc(`oop:${m.id}`)},${esc(tid)},${esc(m.id)},'tennis-europe',${esc(date)},${payload({date,time:m.time||'',notBefore:Boolean(m.notBefore),afterRest:Boolean(m.afterRest),followedBy:Boolean(m.followedBy),scheduleQualifier:m.scheduleQualifier||'',relativeMatchNumber:m.relativeMatchNumber||null,relativeFromTime:m.relativeFromTime||'',court:m.court||'',courtMatchNumber:m.courtMatchNumber||null,event:m.event||'',round:m.round||'',status:m.status,sourceUrl:m.sourceUrl})});`);
 ...
 for(let pIndex=0;pIndex<(m.players||[]).length;pIndex++){const p=m.players[pIndex],source=String(p.id||''),team=teamByPlayer.get(source)??(pIndex<2?0:1),key=`${team}:${source}:${pIndex}`;s.push(`INSERT OR REPLACE INTO match_participants(match_id,participant_key,source_player_id,display_name,normalized_name,nationality,team_index,is_winner,payload) VALUES(${esc(m.id)},${esc(key)},${esc(source)},${esc(p.name)},${esc(norm(p.name))},${esc(p.nationality||'')},${team},${winner.has(source)?1:0},${payload(p)});`)}
}
