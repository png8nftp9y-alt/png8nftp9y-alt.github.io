...
const [manifest,players,tournaments,entries,schedules,matches,results,legacy,playerConfig]=await Promise.all([
...
const sql=['PRAGMA foreign_keys=ON;','DELETE FROM app_matches;','DELETE FROM app_tournaments;','DELETE FROM app_players;','DELETE FROM results;','DELETE FROM matches;','DELETE FROM schedules;','DELETE FROM entries;','DELETE FROM tournaments;','DELETE FROM players;'];
...
const counts={...manifest.counts,appPlayers:appPlayers.length,appTournaments:appTournaments.length,appMatches:appMatches.length};
