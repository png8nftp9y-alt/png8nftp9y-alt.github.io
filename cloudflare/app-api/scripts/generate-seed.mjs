...
const sql=['PRAGMA foreign_keys=ON;','DELETE FROM results;','DELETE FROM matches;','DELETE FROM schedules;','DELETE FROM entries;','DELETE FROM tournaments;','DELETE FROM players;'];
...
sql.push(`INSERT OR REPLACE INTO generations(id,generated_at,schema_version,status,counts_json) VALUES('current',${esc(manifest.generatedAt)},${esc(manifest.version)},'green',${esc(JSON.stringify(manifest.counts))});`);
