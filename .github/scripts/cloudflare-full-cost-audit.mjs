...
const databaseId=process.env.CLOUDFLARE_D1_DATABASE_ID||safeList(databases.result)[0]?.uuid;
let persisted={success:false,reason:'database_not_found'};
if(databaseId){const path=`/d1/database/${databaseId}/query`,created=await api(path,{method:'POST',body:{sql:'CREATE TABLE IF NOT EXISTS cloudflare_usage_snapshots (id TEXT PRIMARY KEY, generated_at TEXT NOT NULL, cycle_start TEXT NOT NULL, cycle_end TEXT NOT NULL, payload TEXT NOT NULL)'}});if(created.success){const sql=`INSERT INTO cloudflare_usage_snapshots(id,generated_at,cycle_start,cycle_end,payload) VALUES('current',?1,?2,?3,?4) ON CONFLICT(id) DO UPDATE SET generated_at=excluded.generated_at,cycle_start=excluded.cycle_start,cycle_end=excluded.cycle_end,payload=excluded.payload`;persisted=await api(path,{method:'POST',body:{sql,params:[snapshot.generatedAt,from,target,JSON.stringify(snapshot)]}})}else persisted=created}
console.log('CLOUDFLARE_USAGE_SNAPSHOT='+JSON.stringify(snapshot));
console.log('CLOUDFLARE_USAGE_PERSISTED='+JSON.stringify({success:persisted.success,status:persisted.status,errors:persisted.errors||[],databaseId:databaseId||null}));
